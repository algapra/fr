...
const NewMembers:React.FC<TotalMembersProps> = ({dataCount}) => {
  return (
    <Card>
      <CardContent >
        <CustomAvatar skin='light'variant='rounded' sx={{ mr: 1.5, mb:1.5,backgroundColor: '#feede2', padding: '0.5rem', height: 50, width: 50, color: 'orange' }}>
          <Icon icon='tabler:user' fontSize='5rem'  />
        </CustomAvatar>
        <Box sx={{ gap: 2, mb: 1.5, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <Typography sx={{ fontWeight: 600, fontSize: 18}}>New Member</Typography>
          </div>
        </Box>
