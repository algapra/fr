...
export class AddPlanColumnOnTransaction1715938337549
  implements MigrationInterface
{
  name = 'AddPlanColumnOnTransaction1715938337549';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`transaction_entity\` ADD \`paymentGateway\` varchar(255) NULL DEFAULT 'Midtrans'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`transaction_entity\` ADD \`paymentGatewayData\` text NOT NULL DEFAULT '{}'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`transaction_entity\` DROP COLUMN \`paymentGatewayData\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`transaction_entity\` DROP COLUMN \`paymentGateway\``,
    );
  }
}
