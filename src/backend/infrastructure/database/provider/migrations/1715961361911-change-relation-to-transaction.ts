...
export class ChangeRelationToTransaction1715961361911
  implements MigrationInterface
{
  name = 'ChangeRelationToTransaction1715961361911';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`transactions\` CHANGE \`username\` \`companyId\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`transactions\` CHANGE \`paymentGateway\` \`paymentGateway\` varchar(255) NULL DEFAULT 'midtrans'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`transactions\` DROP COLUMN \`companyId\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`transactions\` ADD \`companyId\` varchar(36) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`transactions\` ADD \`companyId\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`transactions\` CHANGE \`paymentGateway\` \`paymentGateway\` varchar(255) NULL DEFAULT ''Midtrans''`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`transactions\` CHANGE \`companyId\` \`username\` varchar(255) NOT NULL`,
    );
  }
}
